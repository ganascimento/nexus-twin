...
class WarehouseAgent:
    ...
    async def _build_world_state_slice(self, current_tick: int) -> WorldStateSlice:
        ...
        partner_factories = await factory_repo.list_partner_for_warehouse(self._entity_id)
        pending_orders = await order_repo.get_pending_for_target(self._entity_id)
        active_events = await event_repo.get_active_for_entity("warehouse", self._entity_id)
