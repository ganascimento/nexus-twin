...
| 28  | resilience_and_chaos         | done | 4 gaps: retry backoff after rejection, breakdown roll mid-route, chaos events for factories/stores, reroute on route_blocked |
| 29  | integration_tests_full_cycle | done | 71 integration tests simulação / 588 unit tests / 659 total passando. 7 bugs reais corrigidos: age_ticks freeze, transaction snapshot, cargo sem origin/destination, send_stock semântica invertida, consume_reserved ausente, factory sem atomic_reserve_stock, is_trip_blocked orphan state, low_stock_trigger durante backoff. Cenários 1-19 cobertos. |
| 30  | integration_tests_invariants_and_edges | done | 106 sim integration tests / 588 unit / 80 other integration (774 total). 35 novos testes (Blocos A-H) + 3 testes fortalecidos (Bloco G). BUG-30-1..4 corrigidos + 2 extras descobertos durante a execução. |
| 31  | langfuse_observability       | done | Langfuse self-hosted plugado via callback do LangChain, opt-in por env var. 6 serviços no docker-compose (web/worker/postgres/clickhouse/redis/minio). 19 testes novos. 839 total passando. README com setup. |
...
- [28_resilience_and_chaos] Gap C (factory/store chaos triggers) parcialmente resolvido em F26 para warehouse/store. F28 adicionou factory event triggers no engine. `DecisionEffectProcessor` ganhou handlers: `stop_production`, `alert_breakdown`, `reroute`. `route_repo` adicionado como parâmetro opcional do processor.
- [28_resilience_and_chaos] `roll_breakdown` usa `random.random()` — não determinístico por design. Testes mockam `roll_breakdown` no engine para controlar resultado. Engine simplifica route_blocked: todos os trucks IN_TRANSIT são notificados (matching preciso rota×área pode ser refinado depois).
- [29_integration_tests_full_cycle] Migration atualizada com colunas faltantes dos models (schema em fase de evolução rápida): `trucks.maintenance_start_tick`, `trucks.maintenance_duration_ticks`, `pending_orders.triggered_at_tick`, `routes.order_id` (+ FK `fk_route_order`). `events.source` ampliado de `varchar(20)` → `varchar(50)` (literal `"decision_effect_processor"` tem 27 chars).
- [29_integration_tests_full_cycle] Conftest `seeded_simulation_client` — trocado `DELETE FROM` por `TRUNCATE ... CASCADE` (FK circular trucks↔routes impedia DELETE sequencial). `_override_get_db` agora usa session própria com commit/rollback (API não via dados injetados via test_session).
- [29_integration_tests_full_cycle] `RoutingFakeLLM` no conftest roteia respostas mockadas por `entity_id` (detectado no system prompt via `{entity_id}` placeholder) ou por `agent_type` (palavra-chave "Fábrica"/"Armazém"/"Loja"/"Caminhão" no prompt). Resolve o problema de agentes simultâneos consumindo queue FIFO compartilhada.
- [29_integration_tests_full_cycle] `RefreshingSession` no conftest: wrapper que reabre sessão após commit/rollback, resolvendo snapshot antigo do test_session que não via commits do engine.
- [29_integration_tests_full_cycle] BUG real em F27: `OrderRepository.increment_all_age_ticks` só incrementava status pending/confirmed, congelando age_ticks em 'rejected'. Sem isso, retry_after_tick nunca era alcançado. Adicionado 'rejected' ao set.
- [29_integration_tests_full_cycle] BUG real em F24: `DecisionEffectProcessor._handle_accept_contract` criava cargo sem `origin_type`, `origin_id`, `destination_type`, `destination_id` — TruckCargo Pydantic model rejeitava. Adicionados todos os campos.
- [29_integration_tests_full_cycle] BUG real em F24: `_handle_send_stock` criava order com requester=factory/target=warehouse (inverso da convenção store→wh), o que fazia `_handle_accept_contract` inverter origin/dest. Padronizado: `requester` = quem recebe.
- [29_integration_tests_full_cycle] BUG real em F26: `_apply_physics` na chegada só incrementava destino mas não decrementava origem — warehouse/factory stock nunca baixava após delivery. Adicionado `consume_reserved` em WarehouseRepository e FactoryRepository; engine chama em origin na arrival.
- [29_integration_tests_full_cycle] BUG real em F24 (exposto no review pós-done): `_handle_send_stock` não reservava estoque da factory. No delivery, `consume_reserved` fazia `factory.stock_reserved` ficar negativo (stock_reserved era 0, ficava -qty). Fix: `FactoryRepository.atomic_reserve_stock` adicionado; `_handle_send_stock` reserva antes de criar order; se order já existe (de request_resupply), reserva e marca order como confirmed.
- [29_integration_tests_full_cycle] BUG real em F20/F27 (exposto no review): `is_trip_blocked` (degradation ≥ 0.95) deixava truck em estado orphan — status=idle mas com cargo e active_route_id preservados. Fix: engine agora marca route como `interrupted`, limpa `cargo` e `active_route_id`, transiciona para `broken` e emite `truck_breakdown` event.
- [29_integration_tests_full_cycle] BUG real em F25/F28 (exposto por canary test no backoff): engine disparava `LOW_STOCK_TRIGGER` mesmo com rejected order em backoff window, causando múltiplas decisões do store agent durante a janela. Fix: `OrderRepository.has_order_in_pipeline` considera pending + confirmed + rejected-em-backoff; engine checa antes de firing low_stock trigger.
- [29_integration_tests_full_cycle] Canary test pattern: para verificar "agente NÃO foi acordado durante janela X", programar `make_entity_routing_llm(**{entity_id: [response, response, ...]})` com respostas que normalmente seriam usadas; se o queue sair intacto após a janela, prova que o agente não foi triggered. Detecta false-positive em controles negativos.
- [29_integration_tests_full_cycle] `RoutingFakeLLM` estendida: agora suporta routing por `entity_id` (detecta `` `entity_id` `` no prompt) além de `agent_type` (keyword). `make_entity_routing_llm(**{id: [...]})` e `make_combined_routing_llm(by_entity=..., by_agent=...)` resolvem concorrência entre múltiplos stores/warehouses ativos no mesmo tick.
- [30_integration_tests_invariants_and_edges] BUG-30-1 corrigido: `OrderRepository.bulk_cancel_by_requester` e `bulk_cancel_by_target` passam a retornar `List[CancelledOrderInfo]` (dataclass) descrevendo orders confirmed antes do cancelamento. Services `Store/Warehouse/Factory.delete` chamam `release_reserved` nos targets/requesters de orders confirmed antes de deletar — evita zombie `stock_reserved` após cascata.
- [30_integration_tests_invariants_and_edges] `WarehouseRepository.delete` agora remove FK rows em `factory_partner_warehouses` antes do delete principal — requerido para API DELETE retornar 200 sem ForeignKeyViolationError.
- [30_integration_tests_invariants_and_edges] BUG-30-3 corrigido: `SimulationEngine._apply_physics` delivery path verifica existência do destino/origem antes de `update_stock`; se None, marca `route.status='interrupted'`, cancela order, limpa cargo/active_route_id, libera reserva no origin se ainda existir, transiciona truck para `idle` com warning — sem crash.
- [30_integration_tests_invariants_and_edges] BUG-30-2 corrigido: `_handle_accept_contract` ganhou guards para `truck.status in {broken, maintenance}` e `order.status in {delivered, cancelled}` — retorna cedo sem side effects. `assign_route` agora é envolvido em try/except `ConflictError` com log.
- [30_integration_tests_invariants_and_edges] `DecisionEffectProcessor.process` usa `session.begin_nested()` (SAVEPOINT) em torno do handler. Exceptions causam rollback do savepoint, preservando a linha de `agent_decisions` (já persistida antes) mas revertendo efeitos colaterais parciais (ex: stock reservado mas truck dispatch falhou).
- [30_integration_tests_invariants_and_edges] `TruckDecision` aceita `"hold"` como ação válida. Corrige gap em que o truck agent, acordado por `truck_arrived`, não conseguia responder "nada a fazer" sem falhar na validação Pydantic. Alinha com as demais guardrails (store/warehouse/factory já aceitavam hold).
- [30_integration_tests_invariants_and_edges] Invariantes de conservação usam `(origin.stock - origin.stock_reserved) + cargo + destination.stock` em vez de `origin.stock + cargo + destination.stock`. Razão: engine usa Accounting B (consume_reserved na arrival), então `stock_reserved` representa o cargo em trânsito "em espelho"; somar origin.stock + cargo double-counta durante o trânsito.
- [31_langfuse_observability] `langfuse>=2.60` — `langfuse` 4.x já está disponível; o módulo tenta `from langfuse.langchain import CallbackHandler` (novo) com fallback para `from langfuse.callback import CallbackHandler` (2.x). Testes usam `patch.object(module, "CallbackHandler")` e `importlib.reload` para resetar o cache module-level entre testes.
- [31_langfuse_observability] Helper `build_invoke_config(trigger)` centralizado em `observability/langfuse.py` (não em `agents/base.py`) — cada um dos 4 `run_cycle` só importa e chama; evita duplicar ~15 linhas em cada agent.
- [31_langfuse_observability] `_tag_trace_safely(**kwargs)` em `agents/base.py` — wrapper defensivo que busca handler via `get_callback_handler()`, verifica `hasattr(handler, "update_current_trace")`, e swallow qualquer exceção. Usado no `_act_node` para tagging de erro e fast-path sem risco de quebrar fluxo do agente.
- [31_langfuse_observability] `extract_session_id(trigger)` retorna `None` para `order_id` falsy (inclui `0`, `None`, `""`) — cobre payloads inconsistentes. Coerção para `str(...)` permite `order_id` inteiro do mock tests.
- [31_langfuse_observability] Docker compose com 6 serviços Langfuse isolados dos serviços Nexus Twin (postgres/redis separados) — permite reset independente (`docker compose rm -svf langfuse-*`) sem perder dados da simulação. Credenciais internas (NEXTAUTH_SECRET, SALT, ENCRYPTION_KEY) com placeholders documentados como "change-in-prod".
